...
import org.pwss.model.service.request.monitored_directory.GetDirectoryByIdRequest;
...
public class MonitoredDirectoryService {
    ...
    /**
     * Retrieves a monitored directory by its ID by sending a request to the MONITORED_DIRECTORY_BY_ID endpoint.
     *
     * @param id The ID of the monitored directory to retrieve.
     * @return A MonitoredDirectory object representing the monitored directory with the specified ID.
     * @throws MonitoredDirectoryByIdException If the retrieval attempt fails due to various reasons such as invalid credentials, monitored directory not found, or server error.
     * @throws ExecutionException              If an error occurs during the asynchronous execution of the request.
     * @throws InterruptedException            If the thread executing the request is interrupted.
     * @throws JsonProcessingException         If an error occurs while serializing the request body or deserializing the response body.
     */
    public MonitoredDirectory getDirectoryById(int id) throws MonitoredDirectoryByIdException, ExecutionException, InterruptedException, JsonProcessingException {
        String body = objectMapper.writeValueAsString(new GetDirectoryByIdRequest(id));
        HttpResponse<String> response = PwssHttpClient.getInstance().request(Endpoint.MONITORED_DIRECTORY_BY_ID, body);

        return switch (response.statusCode()) {
            case 200 -> objectMapper.readValue(response.body(), MonitoredDirectory.class);
            case 401 ->
                    throw new MonitoredDirectoryByIdException("Get monitored directory by ID failed: User not authorized to perform this action.");
            case 404 ->
                    throw new MonitoredDirectoryByIdException("Get monitored directory by ID failed: Monitored directory not found.");
            case 500 ->
                    throw new MonitoredDirectoryByIdException("Get monitored directory by ID failed: An error occurred on the server while attempting to retrieve the monitored directory.");
            default -> null;
        };
    }
    ...
    /**
     * Updates an existing monitored directory by sending a request to the MONITORED_DIRECTORY_UPDATE endpoint.
     *
     ...
     * @throws UpdateMonitoredDirectoryException If the update attempt fails due to invalid input, unauthorized access, or server error.
     * @throws ExecutionException                If an error occurs during the asynchronous execution of the request.
     * @throws InterruptedException              If the thread executing the request is interrupted.
     * @throws JsonProcessingException           If an error occurs while serializing the request body.
     */
    public boolean updateMonitoredDirectory(long id, boolean isActive, String notes, boolean includeSubDirs) throws UpdateMonitoredDirectoryException, JsonProcessingException, ExecutionException, InterruptedException {
        String body = objectMapper.writeValueAsString(new UpdateDirectoryRequest(id, isActive, notes, includeSubDirs));
        HttpResponse<String> response = PwssHttpClient.getInstance().request(Endpoint.MONITORED_DIRECTORY_UPDATE, body);
